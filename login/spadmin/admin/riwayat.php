  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      <div class="container-fluid">
        ...
        $cek = mysqli_query($koneksi, "SELECT * FROM diagnosa WHERE id_diagnosa='$id_diagnosa'");
        if(mysqli_num_rows($cek) == 0){

          
        }else{
          $delete = mysqli_query($koneksi, "DELETE FROM diagnosa WHERE id_diagnosa='$id_diagnosa'");
          if($delete){
            
              echo "<script>alert('Data Berhasil di hapus gan!'); window.location = 'data_riwayat.php'</script>";
      ...
      ?>
               ...
               <?php
                                                  include'koneksi.php';
                                                  $no = 1;
  $sql = mysqli_query($koneksi, "SELECT * from riwayat INNER JOIN  kerusakan ON riwayat.kerusakan = kerusakan.kerusakan WHERE id_user='$_SESSION[id_user]'");
  // $sql = mysqli_query($koneksi, "SELECT * from gejala, riwayat INNER JOIN  kerusakan ON riwayat.kerusakan = kerusakan.kerusakan WHERE id_user='$_SESSION[id_user]'");
                     ...
                     <td><left><?php echo $data['tgl_diagnosa'];?></left></td>
                      <td><left><?php echo $data['kerusakan'];?></center></td>
                        <!-- <td><left><?php echo $data['solusi'];?></center></td> -->
                        <td><left></center></td>
                                                <td>
                                                  <div class="btn-group">
                                                    &nbsp;
                                                    <!-- <a class="btn btn-danger" onclick="return confirm('Apakah Anda Yakin Ingin Menghapus Data ini ?')"  href="data_riwayat.php?aksi=hapus&id_diagnosa=<?php echo $data['id_diagnosa']; ?>" class="btn btn-sm btn-danger" title=""><i class="fa fa-trash">Hapus</i></a> -->
                                                    <a class="btn btn-danger" onclick="return confirm('Apakah Anda Yakin Ingin Menghapus Data ini ?')"  href="data_riwayat.php?aksi=hapus&id_diagnosa=<?php echo $data['id']; ?>" class="btn btn-sm btn-danger" title=""><i class="fa fa-trash">Hapus</i></a>
                                                  </div>
                                                </td>
                                            </tr>
            <!-- Edit Modal !-->
                                    ...
                                    if (isset($_POST['simpan'])){
                                      ...
                                      if($tambah){
                                        ...
                                        alert('Input Data Berhasil');
                                          document.location.href="data_gejala.php";
                                         </script>
                                         <?php
                                        }else{
